...
import menu_dataes from "@/data/menu/menu-data-es";
...
const MenuTwo = () => {
  ...
  const idiom = t('lang');
  const menudata = idiom == 'ES' ? menu_dataes : menu_data
  return (
    <>
      <ul>
        {menudata.map((item) => (
          ...
          >
            ...
            {item?.hasDropdown === true && item?.submenus?.length && (
              <ul
                className={`submenu ${item?.lastDropdown === true ? "last-children" : ""
                  }`}
              >
                {item?.submenus?.map((dropdownItem, index) => (
                  <li key={index} className={`menu-item-has-children ${dropdownItem?.megaMenu === true ? "has-arrow" : ""}`}>
